...
class Goodsitem extends CI_Model
{
	...
	{ 
		...
		,PricePerUnit
		,StockAmount
		,ifnull((SELECT sum(Amount) as Used FROM tbl_GoodsOrderDetail where GoodsItemID = ItemMaster.GoodsItemID group by GoodsItemID),'ยังไม่มีการซื้อ') as Used
		,ifnull((StockAmount-(SELECT sum(Amount) as Used FROM tbl_GoodsOrderDetail where GoodsItemID = ItemMaster.GoodsItemID group by GoodsItemID)),'ยังไม่มีการซื้อ') as Available
